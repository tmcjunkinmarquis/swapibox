...
import { apiCall } from '../../apiCalls/filmCrawlApiCall';
...
class FilmCrawl extends Component {
  ...
  cleaner = (filmData) => {
    const { title, release_date, opening_crawl } = filmData  //just need opening crawl, title, release date
  ...
  async componentDidMount() {
    ...
    const filmData = await apiCall(dynamicVariable)  //just need opening crawl, title, release date
